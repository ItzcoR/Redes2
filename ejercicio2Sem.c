...
#define SEM_INCREMENTO 4
#define SEM_PRODUCT 5
#define SEM_CONS 6
...
int num_consumos;
const int num_producciones_x_productor = 12;
...
int main(int argc,char *argv[])
{
	...
	llave = ftok("bin/ls",'k');
	if((semid = semget(llave,7,IPC_CREAT | 0600)) == -1)
	{
	...
	if( semctl(semid,SEM_PRODUCT,SETVAL,2) == -1{

	} //Abrimos el semaforo del lado de los productores
	if(semctl(semid,SEM_CONS,SETVAL,0)  ==-1){

	} //Cerramos el semforo del lado de los consumidores
	if(semctl(semid,PRODUCTOR1,SETVAL,1) ==-1){

	} // abrimos semáforo PRODUCTOR
	if(semctl(semid,PRODUCTOR2,SETVAL,1) ==-1){

	} // abrimos semáforo PRODUCTOR
	if(semctl(semid,CONSUMIDOR1,SETVAL,0) ==-1){

	} //cerramos semáforo CONSUMIDOR
	if(semctl(semid,CONSUMIDOR2,SETVAL,0) ==-1){

	} //cerramos semáforo CONSUMIDOR
	if(semctl(semid,SEM_INCREMENTO,SETVAL,1) ==-1){
		...
		while(TRUE)
		{
			...
			if ((semctl(semid,5,GETVAL)) > 0) {
				...
				if((semctl(semid,semaforos_productores[indice_seccion],GETVAL,0)) > 0)
				{
					...
					printf("productor:%d ,se produjo en: %d,  produzco: %s, numero de producciones : %d\n"
...
//		consumio = FALSE;
		...
		while(TRUE)
		{
			if(indice_seccion >= num_secciones)
				indice_seccion = 0;
			if (num_consumos >= NUM_PRODUCTORES*num_producciones_x_productor) {
				break;
			}
			if ((semctl(semid,6,GETVAL)) > 0) {
				cerramos_semaforo(6);
